...
public record HospitalPolicyInfoResponse(
        ...
        @Schema(description = "환자별 허용 보호자 수", example = "1L") Long maxGuardians){
    public static HospitalPolicyInfoResponse fromEntity(HospitalPolicy policy) {
        return new HospitalPolicyInfoResponse(policy.getReserveDayOffset(), policy.getCutoffTime(), policy.getMaxGuardians());
    }
}
